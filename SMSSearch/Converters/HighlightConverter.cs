    ...
    public class HighlightConverter : IMultiValueConverter
    {
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            try
            {
                ...
                // Read value exclusively from underlying row data to avoid stale UI readings
                // during UI virtualization recycling (scrolling).
                ...
                else if (values.Length >= 6 && values[4] is DataGridColumn col && values[5] != null && values[5] != System.Windows.DependencyProperty.UnsetValue)
                {
                    ...
                    if (!string.IsNullOrEmpty(propName))
                    {
                        // Using TypeDescriptor works for both POCO and CustomTypeDescriptor
